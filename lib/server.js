...
const options = {
  swaggerDefinition: {
    ...
    definitions: {
      category_request: {
        properties: {
          name: { type: 'string', example: 'mythical_weapons', required: true },
          display_name: {
            type: 'string',
      ...
      categories: {
        properties: {
          count: {
            type: 'number',
            example: categories.database.length,
            required: true,
          },
          results: {
            type: 'array',
            example: categories.database,
            required: true,
          },
        },
      },
      ...
      products: {
        properties: {
          count: {
            type: 'number',
            example: products.database.length,
            required: true,
          },
          results: {
            type: 'array',
 ...
 * @returns {products_response.model} 200 - the matching product object
 * @returns {Error} 500 - Unexpected error
 */
 ...
 * @param {product_request.model} category.body.required - object with properties to update existing product with
