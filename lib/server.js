...
// let options = require('../docs/swagger.json');
const options = {
  swaggerDefinition: {
    info: {
    ...
    definitions: {
      catReq: {
        properties: {
          name: { type: 'string', example: 'mjolnir', required: true },
          display_name: { type: 'string', example: 'Mjolnir', required: true },
          description: {
            type: 'string',
            example:
 ...
 * @returns {Array.<catRes>} 200 - An object containing an array of categories and the array count
 ...
 * @returns {object} 200 - the matching category object
 * @returns {Error} 500 - Unexpected error
 */
 ...
 * @param {catReq.model} category.body.required - the new category
 * @category retrieveFooInfo
 * @produces application/json application/xml
 * @consumes application/json application/xml
 * @returns {catRes.model} 201 - the created category object
 * @returns {Error.model}  default - Unexpected error
 * @returns {Array.<category>} category - Some description for category
 ...
 * @route POST /products
 * @param {object} product - product object to add to the database
 * @returns {object} 201 - the added product object
 * @returns {Error} 500 - Unexpected error
 */
