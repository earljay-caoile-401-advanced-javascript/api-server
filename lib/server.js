...
function getAllCategories(req, res, next) {
  categories
    ...
    .then(results => {
      if (req.query) {
        results = results.filter(cat => Object.keys(req.query).every(key => cat[key] === req.query[key]));
      }
      let output = {
...
function getAllProducts(req, res, next) {
  products
    ...
    .then(results => {
      if (req.query) {
        results = results.filter(prod => Object.keys(req.query).every(key => prod[key] === req.query[key]));
      }
      let output = {
        count: results.length,
        results,
